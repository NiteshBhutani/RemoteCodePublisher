...
void CodeAnalysisExecutive::processCsharpFiles(bool showProc) {
...
//----< Function to do Code Analysis -> Type Analysis -> Dependency Ananlysis-> Code Publishing >-------------------------------
/* This is the function called by the server to do all the operation
*/

int CodeAnalysisExecutive::executeCodePublishing(int argc, char* argv[]) {
	using namespace CodeAnalysis;
	...
	try {
		...
		DepAnal depAnal(exec.getFileMap(), exec.numDirs(), exec.numFiles(), ta.returnTypeTable());
		...
		Publisher::CodePublisher cp(argv[1], argv[0]);
		succeeded = cp.handleCommandLineOption(argc, argv);
		if (!succeeded) { return 1; }
		cp.buildFileMap();
	...
	{
		std::cout << "\n\n  caught exception in while publishing code :-  " + std::string(except.what()) + "\n\n";
		return 1;
	}
	return 0;
}
