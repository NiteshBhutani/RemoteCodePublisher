...
{  
  ...
  class CppTypedefDefinition : public IRule
  {
  public:
	  bool doTest(const Scanner::ITokCollection* pTc) override
	  {
  ...
  public:
	  ...
	  {  GrammarHelper::showParseDemo("Handle typedef definition: ", *pTc);
		  /*  p_Repos->currentAccess() = Access::publ;
		  std::cout << "top type is" << top->type_;
		  std::string name = (*pTc)[pTc->find("typedef") + 1];
		  std::cout << "typedef name : " << name<<"\n";
		  top->type_ = "typedef";
		  top->name_ = name;
		  top->package_ = p_Repos->package();
		  p_Repos->AST().typeMap()[name] = top;
		  ...
		  */
		  ...
		  ASTNode* pElem = new ASTNode;
		  pElem->type_ = "typedef";
		  //std::cout << "pTc->find('typedef') =" << pTc->find("typedef");
		  pElem->name_ = (*pTc)[pTc->find("typedef") + 2];
		  pElem->package_ = p_Repos->package();
		  pElem->startLineCount_ = p_Repos->lineCount();
		  //pElem->endLineCount_ = 1;
		  pElem->path_ = p_Repos->currentPath();
		  top->children_.push_back(pElem);
		  //p_Repos->AST().add(pElem);
		  //pElem = p_Repos->AST().pop(); //now POP
		  pElem->endLineCount_ = p_Repos->lineCount();
		  //if (pElem->type_ == "class" || pElem->type_ == "struct")
		  //  (pElem->endLineCount_)++;
		  p_Repos->currentAccess() = Access::priv;
		  //std::cout << "Typedef detected";
	  }
  };
  
  ///////////////////////////////////////////////////////////////
  // rule to detect enum statements
  class CppEnumDefinition : public IRule
 {
  public:
	  bool doTest(const Scanner::ITokCollection* pTc) override
	  {
	  ...
	  class CppAliasDefinition : public IRule
	  {
	  public:
		  bool doTest(const Scanner::ITokCollection* pTc) override
		  {
	  ...
	  class HandleAliasDefination : public IAction
	  {
		  Repository* p_Repos;

	  public:
		  HandleAliasDefination(Repository* pRepos)
		  {
			  p_Repos = pRepos;
		  }
		  void doAction(const Scanner::ITokCollection* pTc) override
		  {
			  ...
			  ASTNode* pElem = new ASTNode;
			  pElem->type_ = "alias";
			  pElem->name_ = (*pTc)[pTc->find("using") + 1];
			  pElem->package_ = p_Repos->package();
			  pElem->startLineCount_ = p_Repos->lineCount();
			  pElem->path_ = p_Repos->currentPath();
			  top->children_.push_back(pElem);
